...
class InferenceEngine:
    def __init__(self, model, seed, gpu):
        ...
        sampling_params = SamplingParams(temperature=0.00, top_p=0.9, max_tokens=3000, seed=seed, stop = '*** END')
        LLM = vllm.LLM(
            model=model,
    ...
    def infer(self, prompts, method, output_file):
        ...
        elif method in ["Explain", "CoT", "ConflictLoc", "ConflictLocExact", "ConflictLocClaim", "ConflictLocEvidence", "ConflictLocSoft"]:
            for idx, output in enumerate(outputs):
                result = clean_text(output.outputs[0].text).strip().lower()
                #matches = re.findall(r"final answer:?\s*[\*#]*\s*(yes|no)", result, re.IGNORECASE | re.DOTALL)
                #answer = matches[-1].lower() if matches else ""
                matches = re.findall(r"final answer\s*(?:is|:|=)?\s*[\*#\"'\s]*(yes|no)\b", result, re.IGNORECASE)
                answer = matches[-1].lower() if matches else ""
